...
type Props = {
  ...
  onChangeFilter: (value: PeriodFilterOption) => void;
};

export const PeriodFilter: React.FC<Props> = ({ filter, onChangeFilter }) => {
  const options: PeriodFilterOption[] = ["Dia", "Mes"];
  ...
  const handleSelect = (value: PeriodFilterOption) => {
  ...
  const getIcon = (option: PeriodFilterOption) => {
    return option === "Dia" ? "calendar-today" : "calendar-month";
  };
...
const styles = StyleSheet.create({
  container: { width: 120, marginRight: 8 },
  chip: {
    flexDirection: "row",
    alignItems: "center",
  ...
  chipText: { color: COLORS.darkLetter, fontSize: 16, marginHorizontal: 6, fontFamily: "OpenSans-Light" },
